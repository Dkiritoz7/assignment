...
int main()
{
    ...
    char *arrPtr=&arr[0];
	int wordstart[100]={0};
    char *wordstartPtr=NULL;
	int wordlen[100]={0};
	int sameNumber[100]={0};
	...
	while(*arrPtr!=0)
	{
		if(*arrPtr!=' '&&!ischar)     //判断单词开头
		{
			ischar=true;
			wordstartPtr=arrPtr;
			wordlen[wordcnt]++;
		}
		else if(*arrPtr==' '&&ischar)     //判断单词结束
		{	
			wordcnt++;
			ischar=false;
		}
		...
		else if(*arrPtr!=' '&&ischar)     //计算单词中间的长度
		{
			ischar=true;
			wordlen[wordcnt]++;
		}
	...
	{ 
		...
		{                                  //实现了相同单词的判断同时避免了之后相同单词和上个单词samenumber一样减少比较
		    ...
		    if(flag==1&&sameNumber[i]==0)   
			{
				sameNumber[i]=sameNumber[i]+1; 
	...
	sameNumber[wordcnt-1]=1;
	for(i=wordcnt;i>=0;i--)               //因为要输出相同次数的第一个出现的单词，所以从后面遍历
	{
		if(maxNumber<=sameNumber[i]) 
		{
			maxNumber=sameNumber[i];
			finalNumber=i;
		}
	}
	cout<<wordcnt<<endl;
	...
	cout<<endl;
	cout<<sameNumber[finalNumber]<<endl;
	return 0;
}
